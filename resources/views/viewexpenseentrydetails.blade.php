...
<table class="table">
	...
	<tr>
		...
		<td class="bg-green"><strong>{{$expenseentry->amount}}</strong></td>
	...
	<tr>
	   ...
	   <td><strong><span>{{$expenseentry->type}}</span></strong></td>
...
<tbody>
	@foreach($expenseentrydailyvehicle as $key=>$ev)
      <tr>
		...
		<td>
			<a href="/vehicledetailsshow/{{$ev->vehicleid}}" class="btn btn-primary" target="_blank">DETAILS</a>
		</td>
...
<tbody>
	@foreach($engagedlaboursarr as $key=>$el)
      <tr>
		...
		<td>
			<a href="/dailylabourdetailsshow/{{$el['id']}}" class="btn btn-primary" target="_blank">DETAILS</a>
		</td>
