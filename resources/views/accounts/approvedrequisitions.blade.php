...
<h3 style="text-align: center;color: blue;font-weight: bold;">APPROVED REQUISITIONS</h3>
...
<table class="table table-responsive table-hover table-bordered table-striped datatable2">
	...
	<tbody>
		...
		<tr style="font-size: 12px;">
			  <td><a href="/viewapprovedrequisition/{{$requisition->id}}" target="_blank" class="btn btn-success">{{$requisition->id}}</a></td>
			...
			<td>{{$requisition->created_at}}</td>
			  ...
			  @else
			    <td></td>
			  @endif
              <td><a href="/viewapprovedrequisition/{{$requisition->id}}" target="_blank" class="btn btn-info">VIEW</a></td>
