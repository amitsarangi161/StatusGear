...
<div class="well" >
<form action="/updateexpenseentry/{{$expenseentry->id}}" method="post" enctype="multipart/form-data">
  {{csrf_field()}}
  ...
  <table class="table table-responsive table-hover table-bordered table-striped">
      ...
      <tr>
         ...
         <td>
          <select class="form-control select2 calc" name="employeeid" onchange="getuserproject();" required="" id="employeeid">
            <option value="">Select a User</option>
            @foreach($users as $user)
               <option value="{{$user->id}}" {{ ( $user->id == $expenseentry->employeeid) ? 'selected' : '' }}>{{$user->name}}</option>
      ...
      <tr>
        ...
        <td>AMOUNT</td>
        <td>
           <input type="text" name="amount" id="amount"  placeholder="Enter Amount Here" autocomplete="off" class="form-control" value="{{$expenseentry->amount}}" required="">
        </td>
      </tr>
      <tr>
       ...
       <tr>
          ...
          <td>
            <textarea class="form-control" name="description" required="" placeholder="Enter Expense Description">{{$expenseentry->description}}</textarea>
          </td>
        </tr>
      <tr>
        <td><strong>EDIT REMARKS</strong></td>
      ...
      <tr>
        <td colspan="2" style="text-align: right;"><button type="submit" class="btn btn-success" id="subbutton">UPDATE</button></td>
      </tr>
      ...
      var uid=$("#employeeid").val();
       if(uid!='')
       {
           ...
           $.ajax({
               ...
               success:function(data) { 
                   $.each(data,function(key,value){
                    $("#projectid").append('<option title="'+value.orgname+'" value="'+value.proid+'">'+value.projectname+'</option>');
                   });
                   showclient();
                   getall();
               }
