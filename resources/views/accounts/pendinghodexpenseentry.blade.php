...
<table class="table table-responsive table-hover table-bordered table-striped datatablescroll">
    ...
    @foreach($expenseentries as $expenseentry)
        ...
        <td><a href="/viewdetailshodexpenseentry/{{$expenseentry->id}}" class="btn btn-info">{{$expenseentry->id}}</a></td>
      ...
      @if($expenseentry->projectname!='')
       ...
       @if($expenseentry->status=='PENDING')
        ...
        @endif
         ...
         <td><a href="/viewdetailshodexpenseentry/{{$expenseentry->id}}" class="btn btn-warning">VIEW</a></td>
